...
class Iniciacion_model extends Model
{
    ...
    public static function getItems($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0)
    {
        ...
        A.nivelActual,case when A.pagoQR>0 THEN 'Pago con QR' when LENGTH(A.okPagoDerechos=1) then 'Deposito' ELSE 'Sin Pago' end AS estadotxt,A.docDepositoDer,
        concat(A.nombres,' ',A.apPaterno,' ',COALESCE(A.apMaterno,'')) AS NombreCompleto, A.certificado,A.fechaCertificado,A.numeroCertificado
    ...
    public static function getTramites($ids)
    {
        ...
        foreach ($ids as $apres) {
            $query = DB::select("SELECT concat(A.nombres,' ',A.apPaterno,' ',COALESCE(A.apMaterno,'')) AS NombreCompleto,DATE_FORMAT(A.fechaIniciacion,'%d/%m/%Y') AS fechaCeremonia,certificado,docDepositoDer FROM sgm_tramites_iniciacion A WHERE A.idTramite=$apres");
            $value = $query[0];
    ...
    {
        $qry = "SELECT A.idTramite,A.numTramite,A.nivelActual,A.valle,A.logia,A.logiaName,concat(A.nombres,' ',A.apPaterno,' ',COALESCE(A.apMaterno,'')) As nombre, A.fechaDepositoDer,A.fechaPagoDerechos,A.docDepositoGDR,A.docDepositoGLB,A.fechaIniciacion,
    ...
    public static function getRegistros($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 1)
    {
        ...
        CASE A.task when 0 then D.nivel when 1 then concat('Completar ', D.nivel) when 3 then D.nivel else 'Rechazado' end as nivel,A.antecedentes,A.cuadernillo,A.antecedentes2
...
INNER JOIN sgm_tramites_tipo D ON D.idNivel=A.NivelActual AND D.idTramiteGrado=1 WHERE $cond ORDER BY A.fechaModificacion DESC Limit $inicio,$cantidad ";
    ...
    public static function getTramiteIni($id, $tipo = 0)
    {
        ...
        A.docDepositoDer,B.logia,C.valle,A.cuadernillo,A.antecedentes,A.antecedentes2 FROM sgm_tramites_iniciacion A INNER JOIN sgm_logias B ON A.logia=B.numero INNER JOIN sgm_valles C ON B.valle=C.idValle WHERE A.idTramite=$id";
        } elseif ($tipo == 2) {
...
B.logia,C.valle,A.numCircular,A.fechaCircular FROM sgm_tramites_iniciacion A INNER JOIN sgm_logias B ON A.logia=B.numero INNER JOIN sgm_valles C ON B.valle=C.idValle WHERE A.idTramite=$id";
            ...
            $qry = "SELECT A.idTramite,A.numTramite,A.nivelActual,A.valle,A.logia,A.logiaName,concat(A.nombres,' ',A.apPaterno,' ',COALESCE(A.apMaterno,'')) As nombre,
DATE_FORMAT(A.fechaInfLaboral,'%d/%m/%Y') AS fechaInfLaboral,A.numActaInfLaboral, DATE_FORMAT(A.fechaInfSocial,'%d/%m/%Y') AS fechaInfSocial,A.numActaInfSocial,
B.logia,C.valle,A.numCircular,A.fechaCircular,A.okInformeSocial,A.okInformeLaboral,A.antecedentes,A.antecedentes2 FROM sgm_tramites_iniciacion A INNER JOIN sgm_logias B ON A.logia=B.numero INNER JOIN sgm_valles C ON B.valle=C.idValle WHERE A.idTramite=$id";
        } elseif ($tipo == 4) {
            $qry = "SELECT A.idTramite,A.numTramite,A.nivelActual,A.valle,A.logia,A.logiaName,concat(A.nombres,' ',A.apPaterno,' ',COALESCE(A.apMaterno,'')) As nombre, A.fechaDepositoDer,A.fechaPagoDerechos,A.docDepositoGDR,A.docDepositoGLB,A.fechaIniciacion,
            DATE_FORMAT(A.fechaInfLaboral,'%d/%m/%Y') AS fechaInfLaboral,A.numActaInfLaboral, DATE_FORMAT(A.fechaInfSocial,'%d/%m/%Y') AS fechaInfSocial,A.numActaInfSocial,
            B.logia,C.valle,A.numCircular,A.fechaCircular,A.antecedentes,A.antecedentes2 FROM sgm_tramites_iniciacion A INNER JOIN sgm_logias B ON A.logia=B.numero INNER JOIN sgm_valles C ON B.valle=C.idValle WHERE A.idTramite=$id";
        } else {
        ...
        DATE_FORMAT(A.fechaActaInforme,'%d/%m/%Y') AS fechaActaInforme,A.actaInformePase,DATE_FORMAT(A.fechaDepositoDer,'%d/%m/%Y') AS fechaDepositoDer,A.docDeposito,DATE_FORMAT(A.fechaIniciacion,'%d/%m/%Y') AS fechaIniciacion,
        A.docDepositoDer,B.logia,C.valle,A.antecedentes,A.antecedentes2 FROM sgm_tramites_iniciacion A INNER JOIN sgm_logias B ON A.logia=B.numero INNER JOIN sgm_valles C ON B.valle=C.idValle WHERE A.idTramite=$id";
        }
        $results = DB::select($qry);
        return $results[0];
    }
    ...
    public static function getTramitesListos($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 3, $limite = 0, $dias = 0)
    {
        $inicio = $cantidad * ($pagina - 1); //se debe mejorr la consuta
        $hoy = date("Y-m-d");
        if ($dias > 0) {
    ...
    public static function getNumTramitesListos($palabra = '', $taller = 0, $valle = 0, $paso = 3, $limite = 0, $dias = 0)
    {
        $hoy = date("Y-m-d");
        if ($dias > 0) {
            $auxdias = $dias + 1;
    ...
    public static function getTramitesPagar($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 3, $limite = 0, $dias = 0)
    {
        $inicio = $cantidad * ($pagina - 1); //se debe mejorr la consuta
        $hoy = date("Y-m-d");
        if ($dias > 0) {
        ...
        if($paso==1)
        $campon="'Listo para depositar derechos' as nivel";
        else
        $campon="D.nivel";
        $qry = "SELECT A.idTramite,A.fechaCreacion,A.numTramite,A.foto,A.apPaterno,A.apMaterno,A.nombres,A.fechaModificacion,A.fInsinuacion,B.logia,CONCAT('R.L.S. ',B.Logia) AS nLogia,B.numero,C.valle,
        $campon ,E.circular,A.fechaCircular,A.fechaIniciacion FROM sgm_tramites_iniciacion A LEFT OUTER JOIN sgm_logias B ON (A.logia=B.numero) LEFT OUTER JOIN sgm_valles C ON B.valle=C.idValle
        INNER JOIN sgm_tramites_tipo D ON D.idNivel=A.NivelActual AND D.idTramiteGrado=1 INNER JOIN sgm_tramites_ini_circulares E ON A.numCircular=E.id
        WHERE $cond ORDER BY A.fechaModificacion DESC Limit $inicio,$cantidad ";
        //   echo $qry;
        $results = DB::select($qry);
    ...
    public static function getNumTramitesPagar($palabra = '', $taller = 0, $valle = 0, $paso = 3, $limite = 0, $dias = 0)
    {
        $hoy = date("Y-m-d");
        if ($dias > 0) {
            $auxdias = $dias - 1;
...
FROM sgm_tramites_iniciacion A LEFT OUTER JOIN sgm_logias B ON (A.logia=B.numero) LEFT OUTER JOIN sgm_valles C ON B.valle=C.idValle INNER JOIN sgm_tramites_tipo D ON D.idNivel=A.NivelActual AND D.idTramiteGrado=1 INNER JOIN sgm_tramites_ini_circulares E ON A.numCircular=E.id WHERE $cond ORDER BY A.fechaIniciacion DESC,B.numero ASC Limit $inicio,$cantidad ";
    ...
    public static function getCeremoniasHechas($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 6, $limite = 0)
    {
        $inicio = $cantidad * ($pagina - 1); //se debe mejorr la consuta
        $date = date('Y-m-d');
        $cond = "LENGTH(A.nombres)> 3 AND A.NivelActual=$paso AND (A.pagoQR>0 OR LENGTH(A.docDepositoDer)>1) AND A.fechaIniciacion<='$date' ";
        if ($taller > 0) {
            $cond .= "AND A.logia=$taller ";
        }
...
// echo $qry;
    ...
    { //revisa observaciones a tramite
        $qry = "SELECT A.descripcion,B.documento, concat(B.nombres,' ',B.apPaterno,' ',B.apMaterno) AS profano FROM sgm2_tramites_observaciones A JOIN sgm_tramites_iniciacion B ON A.idTramite=B.idTramite WHERE A.estado<>1 AND B.documento LIKE '$doc%'";
        $results = DB::select($qry);
        return $results;
    }
