...
class Aumento_model extends Model
{
    ...
    public static function getTramites($ids)
    {
        ...
        foreach ($ids as $apres) {
            ...
            FROM sgm_miembros A join sgm_tramites_aumento B on B.idMiembro=A.id WHERE B.idTramite=$apres");
            $value = $query[0];
    ...
    /************************************************** */
    public static function getApredices($pagina, $cantidad,  $taller = 0, $regu = 3, $apremes = 3)
    {
        $inicio = $cantidad * ($pagina - 1); //se debe mejorr la consuta
        $hoy = date("Y-m-d");
    ...
    public static function getNumAprendices($taller = 0, $regu = 3)
    {
        $hoy = date("Y-m-d");
        $newdate = date("Y-m", strtotime("$hoy -$regu months")) . '-01'; //fecha de corte de obolo
        $cond = "A.Grado=1 ";
    ...
    public static function getTramite($id)
    {
        ...
        A.okIniciacion,A.okSolicitud,A.okExamen,A.okTrabajos,okAsistencia,okComision,DATE_FORMAT(A.fechaAumento,'%d/%m/%Y') AS fechaAumento
    ...
    public static function getTramiteAum($id)
    {
        $qry = "SELECT A.idTramite,M.id AS idMiembro,M.NombreCompleto AS apreName,CONCAT('R.L.S',' ',L.logia,' Nro. ',L.numero) AS logiaName,V.valle, A.logia,A.actaIniciacion,
        DATE_FORMAT(A.fechaIniciacion,'%d/%m/%Y') AS fechaIniciacion,A.actaPase,DATE_FORMAT(A.fechaPase,'%d/%m/%Y') AS fechaPase,A.actaExamen,DATE_FORMAT(A.fechaExamen,'%d/%m/%Y') AS fechaExamen,
        A.okIniciacion,A.okSolicitud,A.okExamen,A.okTrabajos,okAsistencia,DATE_FORMAT(A.fechaAumento,'%d/%m/%Y') AS fechaAumento
      FROM sgm_miembros M JOIN sgm_logias L ON M.LogiaActual=L.numero JOIN sgm_valles V ON L.valle=V.idValle JOIN sgm_tramites_aumento A ON M.id=A.idMiembro WHERE A.idTramite=$id";
        $results = DB::select($qry);
        $resu = count($results);
        if ($resu > 0) {
            return $results[0];
    ...
    public static function getRegistros($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 0)
    {
        ...
        $qry = "SELECT A.idTramite,A.idMiembro,A.fechaCreacion,A.fechaModificacion,A.fechaIniciacion,B.logia,CONCAT('R.L.S. ',B.Logia) AS nLogia,B.numero,C.valle,D.nivel, E.NombreCompleto,A.fechaAumento
...
case when A.pagoQR>0 THEN 'Pago con QR' when LENGTH(A.depositoGLB)>5 then 'Pago con deposito' ELSE 'Sin Pago' end AS estadotxt,F.fechaCeremonia,A.fechaAumento
...
WHERE $cond ";
    ...
    public static function getTramitesPagos($pagina, $cantidad, $palabra = '', $taller = 0, $valle = 0, $paso = 0)
    {
        ...
        IFNULL(A.fechaAumento, 'Sin fecha programada') AS fechaAumento,case when A.pagoQR=0 then 'Pago con deposito' ELSE 'Pago con QR' end AS estadopago,A.numeroCertificado,A.fechaCertificado,
        case when A.okDepoGDR=1 then 'Revisado' ELSE 'Sin Revision' end AS estadotxt,A.depositoGDR,A.okDepoGDR
        FROM sgm_tramites_aumento A JOIN sgm_logias B ON (A.logia=B.numero)
        LEFT OUTER JOIN sgm_valles C ON B.valle=C.idValle INNER JOIN sgm_tramites_tipo D ON D.idNivel=A.NivelActual AND D.idTramiteGrado=2 LEFT JOIN sgm_miembros E ON A.idMiembro=E.id
         WHERE $cond ORDER BY A.fechaModificacion DESC Limit $inicio,$cantidad  ";
        //  echo $qry;
        $results = DB::select($qry);
    ...
    public static function getNumTramitesPagos($palabra = '', $taller = 0, $valle = 0, $paso = 0)
    {
        ...
        $qry = "SELECT count(distinct A.idTramite)  as numero FROM sgm_tramites_aumento A JOIN sgm_logias B ON (A.logia=B.numero) JOIN sgm_miembros E ON A.idMiembro=E.id WHERE $cond ";
        //  echo $qry;
        $results = DB::select($qry);
        $resu = $results[0];
        return $resu->numero;
    ...
    public static function getAsistenciaMeses($logia, $idm, $meses, $opcion = 0)
    {
        ...
        if ($opcion == 1) { //si es visita
            ...
            WHERE A.idMiembro=$idm AND (B.numeroActa1>0) AND B.fechaTenida>'$corte'";
        } else {
            $qry = "SELECT count(DISTINCT A.fechaTenida) AS numero FROM sgm_asistencia A JOIN sgm_asistenciadata B ON A.fechaTenida=B.fechaTenida AND A.idLogia=B.idLogia
            WHERE A.idMiembro=$idm AND (B.numeroActa1>0) AND B.idLogia=$logia AND B.fechaTenida>'$corte'";
        }
        $results = DB::select($qry);
    ...
    public static function getTenidasMeses($logia, $meses)
    {
        $hoy = date("Y-m-d");
        $corte = date("Y-m-d", strtotime("$hoy -$meses months")); //fecha de corte de obolo
        $qry = "SELECT  count(B.fechaTenida) AS numero  from sgm_asistenciadata B WHERE B.numeroActa1>0 AND B.idLogia=$logia AND B.fechaTenida>'$corte'";
        $results = DB::select($qry);
        $results = DB::select($qry);
        return $results[0]->numero;
    }
