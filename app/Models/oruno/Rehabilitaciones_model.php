...
class Rehabilitaciones_model extends Model
{
    use HasFactory;
    private static $tipo = 6;
    public $timestamps = false;
    protected $primaryKey = 'id';
    protected $table = 'sgm2_tramites_rehabilitaciones';
    ...
    DATE_FORMAT(A.fechaAprobacionLogia,'%d/%m/%Y') AS fAprobacionLogia ,DATE_FORMAT(A.fechaDeposito,'%d/%m/%Y') AS fechaDeposito,F.miembro
     FROM sgm2_tramites_rehabilitaciones A JOIN sgm_miembros B ON A.idMiembro=B.id JOIN sgm_logias C ON A.idLogia=C.numero JOIN sgm_valles D ON C.valle=D.idValle JOIN sgm_parametros E
     ON A.estado=E.valor AND E.tipo=6 LEFT JOIN sgm_miembrostipo F ON B.Estado=F.idTipo WHERE $cond ORDER BY A.fechaModificacion DESC Limit " . $inicio . "," . $cantidad . ' ';
// echo $qry;
    ...
    public static function getNumDepositos($palabra = '', $taller = 0, $valle = 0, $level = 1)
    {
        ...
        $qry = "SELECT count(A.id) as numero FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON (A.idLogia=B.numero) WHERE $cond ";
        $results = DB::select($qry);
        $resu = $results[0];
        return $resu->numero;
    }
    public static function getMiembros($idl, $palabra = '',$tiempouno,$tiempodos)
    {
        if ($idl > 0) {
            $hoy = date("Y-m-d");
            ...
            $irreg=$tiempodos+$tiempouno;
            $limitetres = date("Y-m", strtotime("$hoy -$irreg months")) . '-01'; //fecha de corte de obolo
            $quite=$tiempodos+6;
            $limitecuatro = date("Y-m", strtotime("$hoy -$quite months")) . '-01'; //fecha de corte de obolo
            $quiteirr=$tiempodos+6+$tiempodos;
            $limitecinco = date("Y-m", strtotime("$hoy -$quiteirr months")) . '-01'; //fecha de corte de obolo
            if (strlen($palabra) > 2) {
    ...
    //           CASE WHEN A.ultimoPago>'$limitedos' AND A.socio=1 THEN 1 WHEN A.ultimoPago>'$limitedos' AND A.socio>1 THEN 3 WHEN A.socio=1 THEN 2 ELSE 4 END AS caso,F.miembro,
    ...
    //           DATE_FORMAT(A.ultimoPago,'%Y-%b') AS ultimoPago,ultimoPago AS ultimoPagoDate,A.LogiaActual,CASE WHEN A.ultimoPago>'$limitedos' THEN 'Mas de 5 años' ELSE 'De 2 a 5 años' END AS tipotxt , CASE WHEN A.socio=1 THEN 'Sin Rehabilitacion' ELSE 'Rehabilitado' END AS obstxt
    // FROM sgm_miembros A JOIN sgm_logias B ON A.LogiaActual=B.numero LEFT JOIN sgm_miembrostipo F ON A.Estado=F.idTipo
    // WHERE B.numero=$idl AND ((A.ultimoPago<'$limite' AND A.Estado=1) OR A.Estado=5) AND A.NombreCompleto like '%$palabra%' AND A.Miembro<>'Ad-Vitam' ORDER BY A.NombreCompleto LIMIT 20";

$qry="SELECT A.id, A.NombreCompleto,A.Grado,CASE A.Grado WHEN 0 THEN 'Ninguno' WHEN 1 THEN 'Aprendiz' WHEN 2 THEN 'Compañero' WHEN 3 THEN 'Maestro' WHEN 4 THEN 'Ex V.M.' END AS GradoActual,
CASE WHEN A.ultimoPago>'2022-09-01' AND A.socio=1 THEN 1 WHEN A.ultimoPago>'2022-09-01' AND A.socio>1 THEN 3 WHEN A.socio=1 THEN 2 ELSE 4 END AS caso,F.miembro,CASE WHEN A.socio=1 THEN 'Sin Rehabilitacion' ELSE 'Rehabilitado' END AS obstxt,
TIMESTAMPDIFF(MONTH, A.ultimoPago, CURRENT_DATE) AS meses,A.Estado,DATE_FORMAT(A.ultimoPago,'%Y-%b') AS ultimoPago,ultimoPago AS ultimoPagoDate,A.LogiaActual
FROM sgm_miembros A JOIN sgm_logias B ON A.LogiaActual=B.numero LEFT JOIN sgm_miembrostipo F ON A.Estado=F.idTipo
WHERE B.numero=$idl AND ((A.ultimoPago<'$limite' AND A.Estado=1 AND A.ultimoPago>='$limitetres') OR (A.Estado=5 AND A.ultimoPago<'$limitecuatro' AND A.ultimoPago>='$limitecinco')) AND A.NombreCompleto like '%$palabra%' ORDER BY A.NombreCompleto LIMIT 20";
            } else {
    //             $qry = "SELECT A.id, A.NombreCompleto,A.Grado,CASE A.Grado WHEN 0 THEN 'Ninguno' WHEN 1 THEN 'Aprendiz' WHEN 2 THEN 'Compañero' WHEN 3 THEN 'Maestro' WHEN 4 THEN 'V.M./Ex V.M.' END AS GradoActual,
    //           CASE WHEN A.ultimoPago>'$limitedos' AND A.socio=1 THEN 1 WHEN A.ultimoPago>'$limitedos' AND A.socio>1 THEN 3 WHEN A.socio=1 THEN 2 ELSE 4 END AS caso,F.miembro,
    //           CASE WHEN A.ultimoPago>'$limitedos' AND A.socio=1 THEN 'Tramite Simple' WHEN A.ultimoPago>'$limitedos' AND A.socio>1 THEN 'Tramite GLB' WHEN A.socio=1 THEN 'Tramite en el Consejo' ELSE 'Tramite GLB y Consejo' END AS casotxt,
    //           DATE_FORMAT(A.ultimoPago,'%Y-%b') AS ultimoPago,ultimoPago AS ultimoPagoDate,A.LogiaActual,CASE WHEN A.ultimoPago<'$limitedos' THEN 'Mas de 5 años' ELSE 'De 2 a 5 años' END AS tipotxt , CASE WHEN A.socio=1 THEN 'Sin Rehabilitacion' ELSE 'Rehabilitado' END AS obstxt
    // FROM sgm_miembros A JOIN sgm_logias B ON A.LogiaActual=B.numero LEFT JOIN sgm_miembrostipo F ON A.Estado=F.idTipo
    // WHERE B.numero=$idl AND ((A.ultimoPago<'$limite' AND A.Estado=1) OR A.Estado=5) AND A.Miembro<>'Ad-Vitam' ORDER BY A.NombreCompleto LIMIT 20";

$qry="SELECT A.id, A.NombreCompleto,A.Grado,CASE A.Grado WHEN 0 THEN 'Ninguno' WHEN 1 THEN 'Aprendiz' WHEN 2 THEN 'Compañero' WHEN 3 THEN 'Maestro' WHEN 4 THEN 'Ex V.M.' END AS GradoActual,
CASE WHEN A.ultimoPago>'2022-09-01' AND A.socio=1 THEN 1 WHEN A.ultimoPago>'2022-09-01' AND A.socio>1 THEN 3 WHEN A.socio=1 THEN 2 ELSE 4 END AS caso,F.miembro,CASE WHEN A.socio=1 THEN 'Sin Rehabilitacion' ELSE 'Rehabilitado' END AS obstxt,
TIMESTAMPDIFF(MONTH, A.ultimoPago, CURRENT_DATE) AS meses,A.Estado,DATE_FORMAT(A.ultimoPago,'%Y-%b') AS ultimoPago,ultimoPago AS ultimoPagoDate,A.LogiaActual
FROM sgm_miembros A JOIN sgm_logias B ON A.LogiaActual=B.numero LEFT JOIN sgm_miembrostipo F ON A.Estado=F.idTipo
WHERE B.numero=$idl AND ((A.ultimoPago<'$limite' AND A.Estado=1 AND A.ultimoPago>='$limitetres') OR (A.Estado=5 AND A.ultimoPago<'$limitecuatro' AND A.ultimoPago>='$limitecinco'))  ORDER BY A.NombreCompleto LIMIT 20";
    ...
    {
        $qry = "SELECT A.idMiembro,B.valle FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON  A.idLogia=B.numero  WHERE A.id=$id";
        $results = DB::select($qry);
        return $results[0];
    }
    public static function getDatosCert($id)
    {
        $qry = "SELECT A.id, A.fechaAprobacion,A.fechaDeposito, B.logia,C.NombreCompleto,D.valle,B.numero,
(CASE C.Grado WHEN 1 THEN 'Aprendiz' WHEN 2 THEN 'Compañero' WHEN 3 THEN 'Maestro' WHEN 4 THEN 'V.M./Ex V.M.' ELSE 'Ninguno' END) AS GradoActual FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON A.idLogia=B.numero JOIN sgm_miembros C ON A.idMiembro=C.id JOIN sgm_valles D ON B.valle=D.idValle
WHERE A.id=$id";
        $results = DB::select($qry);
        return $results[0];
    }
...
//  echo $qry;
    ...
    public static function getNumeroTramitesQR($palabra = '', $valle = 0,$taller = 0,  $level = 1)
    {
        ...
        $qry = "SELECT count(A.id) as numero FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON (A.idLogia=B.numero) WHERE $cond "; //    echo $qry;
        //  echo $qry;
        $results = DB::select($qry);
        $resu = $results[0];
        return $resu->numero;
    ...
    public static function getTramitePago($id, $tipo = 5)
    {
        ...
        FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON A.idLogia=B.numero JOIN sgm_valles C ON B.valle=C.idValle
        WHERE A.id=$id";
    ...
    {
        $qry = "SELECT A.idLogia AS logia, B.valle, DATE_FORMAT(A.ultimoPago,'%d/%m/%Y') AS fechaTramite,A.gradoActual,A.estado FROM sgm2_tramites_rehabilitaciones A JOIN sgm_logias B ON A.idLogia=B.numero WHERE A.id=$id";
        $query = DB::select($qry);
        return $query[0];
    }
