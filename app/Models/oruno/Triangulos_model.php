...
class Triangulos_model extends Model
{
    use HasFactory;
    ...
    public static function getlogiasArray($oriente, $valle = 0, $sulogia = 0)
    {
        ...
        if ($oriente > 0) {
            ...
            $qry = self::select('numero as nlogia', DB::raw("CONCAT(logia,' Nro. ' ,numero) AS logian"))->whereraw($cond)->orderBy('numero', 'asc')->pluck('logian', 'nlogia');
        return $qry;
    }
    public static function getItems($pagina, $cantidad, $palabra = '', $estado = 0, $oriente = 0, $valle = 0)
    {
        $inicio = $cantidad * ($pagina - 1);
        $cond = "A.borrado = '$estado' and A.tipo=2 ";
        if ($oriente > 0)
