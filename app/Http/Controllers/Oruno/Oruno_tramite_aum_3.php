...
class Oruno_tramite_aum_3 extends Controller
{
     ...
     */
    public $idmod = '089';
    public $controlador = 'oruno_tramite_aum_3';
    public $tipoqr = 3;
    ...
    public function get_datos(Request $request)
    {
        ...
        $salida = Aumento_model::getTramitesListos($page, $rows, $palabra, $taller, $valle, 1);
        $total = Aumento_model::getNumTramitesListos($palabra, $taller, $valle, 1);
        $qry2 = (object) ['total' => $total, 'rows' => $salida];
        return response()->json($qry2);
    }
    public function get_ceremonia(Request $request)
    {
        $tram = $request->input('id', 0);
        if ($tram > 0) {
            $salida = Aumento_model::getTramiteAum($tram);
        } else {
    ...
    public function update_tramite(Request $request)
    {
        ...
        if ($id > 0 && $request->hasFile('depositoGDR')) {
            ...
            if (strlen($antef) > 5) {
                ...
                Aumento_model::where('idTramite', $id)->update($data);
