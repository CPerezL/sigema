...
class Oruno_tramite_regu_circulares extends Controller
{
     ...
     */
    public $idmod = '082';
    public $controlador = 'oruno_tramite_regu_circulares';
    public $filename = '';
    public $paginas = 1;
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('oruno.regu_circulares', $data);
    }
    public function get_datos(Request $request)
    {
        ...
        $qry = Regularizacion_circulares_model::getCirculares($page, $rows, Session::get($this->controlador . '.palabra', ''), $log, $val, 2);
        $total = Regularizacion_circulares_model::getNumCirculares(Session::get($this->controlador . '.palabra', ''), $log, $val, 2);
    ...
    public function ver_circular(Request $request)
    {
        ...
        if ($idc > 0) {
            ...
            $salida = ['success' => 'true', 'Msg' => 'Buscando', 'filename' => $filename];
        }
    ...
    {
        $idc = $request->input('idc', 0);
        $info = Regularizacion_circulares_model::getInfo($idc); //datos   que se imprimiran
        $data['fechagen'] = $this->getFechaGen($info);
        $data['circular'] = $info->circular;
        if ($info->valle == 2) {
            $auto = 'GRAN SECRETARIA';
            $region = 'Gran Logia Simbólica del Paraguay';
        } else {
            $auto = 'GRAN SECRETARIA';
            $region = 'Gran Logia Simbólica del Paraguay';}
        $data['region'] = $region;
        ...
        $pdf = PDF::loadView('oruno.pdf_regucirculares', $data);
        ...
        $salida = ['success' => 'true', 'Msg' => 'Creado', 'filename' => $filename];
        return response()->json($salida);
    }
    ...
    {
        $lista = Regularizacion_circulares_model::getLista($idc); //datos   que se imprimiran
