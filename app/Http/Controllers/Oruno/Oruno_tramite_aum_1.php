...
class Oruno_tramite_aum_1 extends Controller
{
     ...
     */
    public $idmod = '083';
    public $controlador = 'oruno_tramite_aum_1';
    public $numExtraT = 6;
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('oruno.aum_1', $data);
    }
    public function get_datos(Request $request)
    {
        $apremes = Config_model::getValue('mesesAumento');
        $apreasis = Config_model::getValue('asisAumento');
        $regu = Config_model::getValue('mesesDeuda');
        $hoy = date("Y-m-d");
        $page = $request->input('page', 1);
        $rows = $request->input('rows', 20);
        $log = Session::get($this->controlador . '.taller', 0);
        $salida = Aumento_model::getApredices($page, $rows, $log, $regu, $apremes);
        $total = Aumento_model::getNumAprendices($log, $regu);
        $ntenidas = Aumento_model::getTenidasMeses($log, $apremes); //24
        foreach ($salida as $key => $ver) { //lista de aprendices de la logia
            $condasis = $apreasis;
            $ord = Aumento_model::getAsistenciaMeses($log, $ver->id, $apremes);
            $ase = Aumento_model::getAsisExtraMeses($ver->id, $apremes);
    ...
    public function get_datos_old(Request $request)
    {
        ...
        $salida = Aumento_model::getApredices($page, $rows, $log, 0, 1, $apremes);
        $total = Aumento_model::getNumAprendices($log, 0, 1);
        ...
        foreach ($salida as $key => $ver) { //lista de aprendices de la logia
            ...
            //--------------------------------------------------
            if ((($salida[$key]->extratemplo >= 60 || $salida[$key]->extratemplo2 >= 60) && ($salida[$key]->ordinaria >= $condasis || $salida[$key]->minimo >= 30))) {
                if ($ver->antOk > 0) {
                    $salida[$key]->antOk = 1;
                } else {
                    $salida[$key]->antOk = 0;
