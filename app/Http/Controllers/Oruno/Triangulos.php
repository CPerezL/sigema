...
use App\Models\oruno\Triangulos_model;
...
class Triangulos extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('oruno.triangulos', $data);
    }
    public function get_datos(Request $request)
    {
        $page = $request->input('page', 1);
        $rows = $request->input('rows', 20);
        $qry = Triangulos_model::getItems($page, $rows, Session::get($this->controlador . '.palabra', ''), Session::get($this->controlador . '.papelera', 0), Session::get($this->controlador . '.oriente', 0), Session::get($this->controlador . '.valle', 0));
        $total = Triangulos_model::getNumItems(Session::get($this->controlador . '.palabra', ''), Session::get($this->controlador . '.papelera', 0), Session::get($this->controlador . '.oriente', 0), Session::get($this->controlador . '.valle', 0));
    ...
    {
        $existe = Triangulos_model::where("numero", $request->input('numero', 0))->exists();
        if ($existe == 1) {
            ...
            if (strlen($log) > 4) {
                ...
                $resu = Triangulos_model::insert($data);
                if ($resu > 0) {
    ...
    public function update_datos(Request $request)
    {
        ...
        if ($id > 0) {
            ...
            $resu = Triangulos_model::where("idLogia", $id)->update($data);
            if ($resu > 0) {
    ...
    { //manda a papelera
        ...
        if ($id > 0) {
            ...
            if ($bor > 0)
                $resu = Triangulos_model::where("idLogia", $id)->update(['borrado' => 0]);
            else
                $resu = Triangulos_model::where("idLogia", $id)->update(['borrado' => 1]);
            if ($resu > 0) {
    ...
    public function convertir(Request $request)
    {
        $id = $request->input('id', 0);
        if ($id > 0) {
            $resu = Triangulos_model::where("idLogia", $id)->update(['tipo' => 1]);
            if ($resu > 0) {
                return response()->json(['success' => 'true', 'Msg' => trans('mess.okchange')]);
            } else {
                return response()->json(['success' => 0, 'Msg' => trans('mess.errchange')]);
