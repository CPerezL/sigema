...
use App\Models\oruno\Aumento_model;
...
class Oruno_tramite_aum_4 extends Controller
{
     ...
     * @return void
     */
    private $urlqr = 'https://www.glsp.py/';
    public $idmod = '077';
    public $controlador = 'oruno_tramite_aum_4';
    ...
    public function get_datos(Request $request)
    {
        ...
        $qry = Certificados_model::getAumentados($page, $rows, Session::get($this->controlador . '.palabra', ''), $log, $val);
        $total = Certificados_model::getNumAumentados(Session::get($this->controlador . '.palabra', ''), $log, $val);
    ...
    public function gen_reporte(Request $request)
    {
        ...
        if ($id > 0) {
            $dcere = Certificados_model::getDatosCertificadoAum($id);
            $nomefile = 'Cert-C-' . str_pad($dcere->numeroCertificado, 7, "0", STR_PAD_LEFT);
    ...
    public function update_data(Request $request)
    {
        ...
        if ($id > 0) {
            $datos = Aumento_model::select('idMiembro', 'fechaCertificado', 'fechaAumento', 'numeroCertificado', 'logia')->find($id);
            $data = array(
                'Grado' => 2,
                'FechaCertificadoAum' => $datos->fechaCertificado,
                'CertificadoAum' => $datos->numeroCertificado,
                'LogiaAumento' => $datos->logia,
                'FechaAumentoSalario' => $datos->fechaAumento,
