...
class Oruno_tramite_aum_certificados extends Controller
{
     ...
     */
    public $idmod = '032';
    public $controlador = 'oruno_tramite_aum_certificados';
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('oruno.aum_certificados', $data);
    }
    public function get_datos(Request $request)
    {
        ...
        $qry = Aumento_model::getTramitesPagos($page, $rows, Session::get($this->controlador . '.palabra', ''), $log, $val, 4);
        $total = Aumento_model::getNumTramitesPagos(Session::get($this->controlador . '.palabra', ''), $log, $val, 4);
    ...
    public function update_ceremonia(Request $request)
    {
        ...
        if ($id1 > 0) {
            ...
            $data = array(
                'certificado' => 1,
                'okDepoGDR' => $request->input('okDepoGDR', 0),
                'codGes' => $codges,
                'numeroCertificado' => $num,
                'fechaCertificado' => $fccc,
                'fechaModificacion' => date('Y-m-d'),
            );
            if ($request->input('okDepoGDR', 0) == 1) {
                $data['nivelActual'] = 5;
            }
