...
class Tramite_retiro_5 extends Controller
{
     ...
     */
    public $idmod = '062';
    public $controlador = 'tramite_retiro_5';
    private $error = '';
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('oruno.retiro_5', $data);
    }
    public function get_datos(Request $request)
    {
        ...
        $salida = Retiro_model::getListaRetirados($page, $rows, Session::get($this->controlador . '.palabra'), $log, $val);
        $total = Retiro_model::getNumeroRetirados(Session::get($this->controlador . '.palabra'), $log, $val);
    ...
    {
        $mes = 'Plazo modificado';
        $id = $request->input('id');
        $task = $request->input('task');
        $okay = $request->input('okEnviar');
        if ($id > 0 && $task > 1) {
            if ($task == 8 && $okay == 1) {
                $data = array(
                    'usuario' => Auth::user()->id,
                    'fechaPlazo' => \DateTime::createFromFormat('d/m/Y', $request->input('fechaPlazo'))->format('Y-m-d'),
                    'fechaModificacion' => date('Y-m-d h:m:s'),
                );
                $resu = Retiro_model::where('id', $id)->update($data);
            }
            if ($resu == 1) {
                if ($task == 8 && $okay == 1) {
                    Logs_model::insertLog('8', Auth::user()->id, 'Actualizacion de plazo', '', 0, 0);
                } else {
                    $mes = 'No encontrado';
                }
                return response()->json(['success' => 'true', 'Msg' => $mes]);
