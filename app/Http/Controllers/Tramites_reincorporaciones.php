...
class Tramites_reincorporaciones extends Controller
{
     ...
     */
    public $idmod = '055';
    public $controlador = 'tramites_reincorporaciones';
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('tramites.reincorporaciones', $data);
    }
    ...
    public function get_miembros(Request $request)
    {
        ...
        if ($id > 0) {
            ...
            $salida = Reincorporacion_model::getMiembros($id, $filtro);
    ...
    public function save_tramite(Request $request)
    {
        if ($request->input('id') > 0) {
            $id = $request->input('id');
            if (Reincorporacion_model::checkTramite($id)) {
                $data = array(
                ...
                $resu = Reincorporacion_model::insertGetId($data);
                if ($resu > 0) {
    ...
    public function cambia_tramite(Request $request)
    {
        ...
        if ($id > 0) {
            ...
            $resu = Reincorporacion_model::where('id', $id)->update($data);
            if ($resu > 0) {
    ...
    public function unset_tramite(Request $request)
    {
        $id = $request->input('id', 0);
        if ($id > 0) {
            $resu = Reincorporacion_model::where("id", $id)->delete();
            if ($resu > 0) {
    ...
    { // funcion que registra el aporte yt actualiza los pagos
        ...
        $ntram = Reincorporacion_model::getDatosTramite($idt);
        $idm = $ntram->idMiembro;
        $val = $ntram->valle;
        // $idm = $tram->idMiembro;
        // $val = self::validar('idValle', Session::get($this->controlador . '.valle'));
        $gestion = Config_model::getValue('gestion');
        $numero = Reincorporacion_model::getFormulario($gestion, $val) + 1;
        //inserta formulario
        $datasf = array(
            'tipo' => 4, //obolos
            'estado' => 4,
        ...
        if (Registros_model::checkRegistro($idf, $idm)) {
            ...
            $miembro = Reincorporacion_model::getDatosMiembro($idm);
    ...
    {
        $mnt = Reincorporacion_model::getDatosCuota($pago, $miembro, $valle);
