...
class Tramites_exa_dos extends Controller
{
     ...
     */
    public $idmod = '087';
    public $controlador = 'tramites_exa_dos';
    ...
    public function index(Request $request)
    {
        ...
        /*     varibles de pagina* */
        return view('tramites.exa_dos', $data);
    }
    public function get_datos(Request $request)
    {
        ...
        $qry = Exaltacion_model::getTramitesListos($page, $rows, Session::get($this->controlador . '.palabra', ''), $log, $val, 1);
        $total = Exaltacion_model::getNumTramitesListos(Session::get($this->controlador . '.palabra', ''), $log, $val, 1);
    ...
    public function update_ceremonia(Request $request)
    {
        ...
        if ($id1 > 0) {
            $idcer = Ceremonias_model::updateCeremonia(Session::get($this->controlador . '.taller', ''), $fc, 3); //devuleve id de la ceremonia
        } else {
            $idcer = 0;
        }
        $datas = array(
    ...
    public function get_tramites(Request $request)
    {
        ...
        if (count($tramites) > 0) {
            $salida = Exaltacion_model::getListaExaltados($tramites);
        } else {
