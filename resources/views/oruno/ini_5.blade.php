...
@section('content')
  ...
  <div class="easyui-layout" data-options="fit:true">
    ...
    <div class="datagrid-toolbar" id="toolbar{{ $_mid }}" style="display:inline-block">
      <div style="float:left;"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="fa fa-upload correcto" onclick="revdatos_133();">Enviar datos de pago derechos</a>
      </div>
  ...
  <script type="text/javascript">
    function revdatos_133() {
      ...
      if (row) {
        $('#fmini5{{ $_mid }}').form('load', '{{ $_controller }}/get_ceremonia?_token={{ csrf_token() }}&id=' + row.idTramite); // load from URL
  ...
  <div id="dlgini5{{ $_mid }}" class="easyui-dialog" style="width:500px;height:auto;" closed="true" buttons="#dlgini5-buttons{{ $_mid }}" data-options="iconCls:'icon-save',modal:true">
    <form id="fmini5{{ $_mid }}" enctype="multipart/form-data" method="post" novalidate>
      <div class="easyui-panel" title="Datos del tramite para Iniciación" style="width:100%;padding:5px;">
        ...
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="logiaName" style="width:100%;" data-options="label:'<b>R:.L:.S:::</b>',readonly:'true',editable:false" labelWidth="160"></div>
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="nombre" style="width:100%" data-options="label:'<b>Profano:<b>',readonly:'true',editable:false" labelWidth="160"></div>
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="numCircular" style="width:100%" data-options="label:'<b>Numero circular:<b>',readonly:'true',editable:false" labelWidth="160"></div>
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="fechaCircular" style="width:100%" data-options="label:'<b>Fecha circular:<b>',readonly:'true',editable:false" labelWidth="160"></div>
      </div>
      <div class="easyui-panel" title="Pago de derechos" style="width:100%;padding:5px;">
        ...
        <label><b>Doc. de deposito o transf. bancaria por derechos de iniciación*:</b></label>
        <input class="easyui-filebox" id="docdeposito" name="docdeposito" style="width:100%" buttonText="Archivo PDF/Imagen" accept=".pdf,.png,.jpeg,.jpg,.gif" value="" required="required" data-options="prompt:'Deposito de derechos'">
      </div>
    </form>
  </div>
    ...
    function pagarqr_{{ $_mid }}() {
      ...
      if (row) {
        if (row.estado == 1) {
          ...
          $('#fmini5{{ $_mid }}').form('load', '{{ $_controller }}/get_ceremonia?_token={{ csrf_token() }}&id=' + row.idTramite); // load from URL
          $('#dlgini5{{ $_mid }}').dialog('open').dialog('setTitle', 'Pago de derechos de Ceremonia');
        }
      } else {
        $.messager.show({
          title: 'Error',
