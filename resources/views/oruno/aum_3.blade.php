...
@section('content')
  ...
  <div class="easyui-layout" data-options="fit:true">
    ...
    <div class="datagrid-toolbar" id="toolbar{{ $_mid }}" style="display:inline-block">
      <div style="float:left;"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="fa fa-check-square fa-lg correcto" onclick="revdatos_133();">Enviar datos de pago de derechos</a>
      </div>
  ...
  <script type="text/javascript">
    function revdatos_133() {
      ...
      if (row) {
        $('#fmaum3{{ $_mid }}').form('load', '{{ $_controller }}/get_ceremonia?_token={{ csrf_token() }}&id=' + row.idTramite); // load from URL
        $('#dlgaum3{{ $_mid }}').dialog('open').dialog('setTitle', 'Registro de deposito');
      ...
      $('#fmaum3{{ $_mid }}').form('submit', {
        ...
        success: function(result) {
          ...
          if (!result.success) {
            ...
            $('#fmaum3{{ $_mid }}').form('clear');
            $('#dlgaum3{{ $_mid }}').dialog('close'); // close the dialog
  ...
  <div id="dlgaum3{{ $_mid }}" class="easyui-dialog" style="width:500px;height:auto;" closed="true" buttons="#dlgaum3-buttons{{ $_mid }}" data-options="iconCls:'icon-save',modal:true">
    <form id="fmaum3{{ $_mid }}" enctype="multipart/form-data" method="post" novalidate>
      <div class="easyui-panel" title="Datos del tramite para Aumento de Salario" style="width:100%;padding:5px;">
        ...
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="apreName" style="width:100%" data-options="label:'<b>A:.M:.:<b>',readonly:'true',editable:false" labelWidth="140"></div>
        <div style="margin-bottom:0px"><input class="easyui-textbox" name="fechaAumento" style="width:100%" data-options="label:'<b>Fecha ceremonia:<b>',readonly:'true',editable:false" labelWidth="140"></div>
      </div>
      <div class="easyui-panel" title="Pago de derechos" style="width:100%;padding:5px;">
        ...
        <label><b>Doc. de deposito o transf. bancaria por derechos de Aumento*:</b></label>
  ...
  <div id="dlgaum3-buttons{{ $_mid }}">
    @if (Auth::user()->permisos == 1)
      <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="savedatos_133();" style="width:90px">Grabar</a>
    @endif
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgaum3{{ $_mid }}').dialog('close');" style="width:90px">Cancelar</a>
  </div>
@endsection
