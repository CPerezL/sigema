...
@section('content')
  ...
  <div id="dlg4{{ $_mid }}" class="easyui-dialog" style="width:450px;height:auto;padding:5px 5px" closed="true" buttons="#dlgl-buttons4{{ $_mid }}" data-options="iconCls:'icon-save',modal:true">
    <form id="fm4{{ $_mid }}" method="post" novalidate>
      ...
      <div style="margin-top:5px"><input class="easyui-datebox" name="fechaSolicitud" style="width:100%" data-options="label:'<b>Fecha de solicitud:</b>'" labelWidth="220" readonly="true"></div>
      ...
      <div style="margin-top:5px"><b>Documento: </b><a href="javascript:void(0)" id="abrir_dep" class="easyui-linkbutton c6" iconCls="icon-print" target="_blank" style="width:300px;margin-left:10px;">Ver documento de solicitud</a></div>
    </form>
  </div>
  ...
  <script type="text/javascript">
    function afi_aprobar() {
      ...
      if (row) {
        if (row.estado == '2') {
          $('#dlg4{{ $_mid }}').dialog('open').dialog('setTitle', 'Revisar y Aprobar Retiro (Quite and Placet)');
          $('#fm4{{ $_mid }}').form('clear');
