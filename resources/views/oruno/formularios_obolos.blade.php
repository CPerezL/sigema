...
@section('content')
  <div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'west',title:'Formularios de pago de obolos',collapsed:false,split:false" style="width:650px;">
      <table id="dgfqr{{ $_mid }}" class="easyui-datagrid" style="width:100%;height:100%;"
        ...
        <thead>
          <tr>
            ...
            <th data-options="field:'numeroMiembros'">N.HH:.</th>
      ...
      <div class="datagrid-toolbar" id="toolbardia{{ $_mid }}" style="display:inline-block">
        <div style="float:left;"><input id="gestform{{ $_mid }}" class="easyui-numberspinner" style="width:80px;" required="required" data-options="min:2021,max:{{ $year }},editable:false,onChange: function(rec){fMecomFormsQR(rec,'gestion');}" value="{{ $year }}">
        </div>
    ...
    <div data-options="region:'center'">
      ...
      <div class="datagrid-toolbar" id="tbptasis{{ $_mid }}">
        ...
        <div style="float:left;"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="fa fa-share-square fa-lg orange" onclick="enviar_mqrform{{ $_mid }}();" id="darasis">Enviar formulario</a></div>
              ...
              if (r) {
                $.post('{{ $_controller }}/remove_obolo', {
                  ...
                  if (!result.success) {
                    ...
                    $('#dgfqr{{ $_mid }}').datagrid('reload').datagrid('getChecked');;
                  }
                }, 'json');
              }
            });
  ...
  <script>
    function ver_qraportantes(valued, valuenum, valuetall) {
      log = $('#filtro{{ $_mid }}').combobox('getValue');
      if (log > 0) {
        if (valued > 0) {
          $('#dgqr{{ $_mid }}').datagrid('reload', {
            key: 1,
          ...
          $('#dgqr{{ $_mid }}').datagrid('getPanel').panel('setTitle', 'Logia Nº' + valuetall + ' Form. Seleccionado Nº ' + valuenum);
        }
      } else {
        $.messager.show({
          title: 'Error',
          msg: '<div class="messager-icon messager-error"></div>Seleccion logia para revisar</div>'
        });
      }
  ...
  <script type="text/javascript">
    function enviar_mqrform{{ $_mid }}() {
      ...
      if (row) {
        if (row.estado == '1') {
          $.messager.confirm('Confirm', '¿Esta seguro de enviar el formulario?, ya no podra añadir mas aportantes despues de enviar',
            function(r) {
              if (r) {
  ...
  <div id="dlg_enviarf{{ $_mid }}" class="easyui-dialog" style="width:400px;height:auto;padding:5px 5px" closed="true" buttons="#enviar-buttons{{ $_mid }}" data-options="iconCls:'icon-man',modal:true">
    <form id="enviarf{{ $_mid }}" method="post" enctype="multipart/form-data" novalidate>
      <div style="margin-top:4px">
        <div style="margin-top:10px">
          <label><b>Comprobante de Deposito Cuenta GLSP</b></label>
          <input class="easyui-filebox" id="fileup" name="fileup" style="width:100%" buttonText="Archivo PDF/Imagen" accept=".pdf,.png,.jpeg,.jpg,.gif" value="" required="required" data-options="prompt:'Deposito GLSP'">
        </div>
        <div style="margin-top:15px">
          <label><b>Comprobante de Deposito Cuenta Salud</b></label>
          <input class="easyui-filebox" id="fileup2" name="fileup2" style="width:100%" buttonText="Archivo PDF/Imagen" accept=".pdf,.png,.jpeg,.jpg,.gif" value="" required="required" data-options="prompt:'Deposito COMAP'">
        </div>
        <div style="margin-top:15px">
          <label><b>*Tiene que enviar los comprobantes de deposito</b></label>
        </div>
      </div>
    </form>
    <div id="enviar-buttons{{ $_mid }}">
      <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="enviando_mqrform{{ $_mid }}();" style="width:90px">Enviar</a>
