...
@section('content')
  ...
  <script type="text/javascript">
    function revDatos_{{ $_mid }}() {
      ...
      if (count > 0) {
        ...
        $('#fmaum2{{ $_mid }}').form('load', '{{ $_controller }}/get_tramites?_token={{ csrf_token() }}' + ids); // load from URL
        $('#dlgauun{{ $_mid }}').dialog('open').dialog('setTitle', 'Registrar pago de derechos de Ceremonia de Aumento');
        url = '{{ $_controller }}/update_ceremonia?_token={{ csrf_token() }}';
      } else {
        $.messager.show({
          title: 'Error',
          msg: '<div class="messager-icon messager-error"></div><div>Seleccione tramite(s) primero</div>'
        });
      }
    }
  ...
  <div id="dlgauun{{ $_mid }}" class="easyui-dialog" style="width:500px;height:auto;" closed="true" buttons="#dlgauun-buttons{{ $_mid }}" data-options="iconCls:'icon-save',modal:true">
    <form id="fmaum2{{ $_mid }}" enctype="multipart/form-data" method="post" novalidate>
      ...
      <div class="easyui-panel" title="Lista de aprendices" style="width:100%;padding:5px;">
        <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="apreName1" style="width:100%;" data-options="label:'<b>1. A:.M:.</b>',readonly:'true',editable:false" labelWidth="80"></div>
        <input type="hidden" name="idMiembro1" value="0" />
        <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="apreName2" style="width:100%;" data-options="label:'<b>2. A:.M:.</b>',readonly:'true',editable:false" labelWidth="80"></div>
        <input type="hidden" name="idMiembro2" value="0" />
        <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="apreName3" style="width:100%;" data-options="label:'<b>3. A:.M:.</b>',readonly:'true',editable:false" labelWidth="80"></div>
        <input type="hidden" name="idMiembro3" value="0" />
        <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="apreName4" style="width:100%;" data-options="label:'<b>4. A:.M:.</b>',readonly:'true',editable:false" labelWidth="80"></div>
