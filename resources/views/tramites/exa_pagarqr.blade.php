...
@section('content')
  ...
  <div class="easyui-layout" data-options="fit:true">
    ...
    <div class="datagrid-toolbar" id="toolbar{{ $_mid }}" style="display:inline-block;">
      <div style="float:left;"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="fa fa-check-square fa-lg correcto" onclick="revDatos_{{ $_mid }}();">Pagar derecho de
          exaltacion</a></div>
  ...
  <script type="text/javascript">
    function revDatos_{{ $_mid }}() {
      ...
      if (row) {
        if (row.estado == 1) {
          ...
          $('#form{{ $_mid }}').form('load', '{{ $_controller }}/get_ceremonia?_token={{ csrf_token() }}&id=' + row.idCeremonia + '&num=1'); // load from URL
    ...
    function abrirPago_128() {
      ...
      if (row) {
        $.post('{{ $_controller }}/get_datos_pagos', {
          ...
          if (ok == 1) {
            var url =
              '{{ $linkiframe }}?entidad={{ $entidad }}&red={{ $linkaccion }}/codigo/' + red + '&ref=' + registro;
            $("#modalpg_128").attr("src", url);
          }
        }, 'json');
      } else {
        $.messager.show({
    ...
    function cerrarPago_128() {
      $('#modalpg_128').attr('src', '');
      $("#mymodal").hide();
    }
  </script>
    ...
    <div id="cc" class="easyui-layout" style="width:100%;height:100%;">
      <form id="form{{ $_mid }}" method="post" novalidate>
        <div data-options="region:'west',split:true" style="width:220px;">
          ...
          <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="fechaCeremonia" style="width:100%;" data-options="label:'<b>Fecha Ceremonia:</b>',readonly:'true',editable:false" labelWidth="120"></div>
          ...
          <div style="margin-bottom:0px;margin-left:5px"><input class="easyui-textbox" name="monto" style="width:100%;" data-options="label:'<b>Importe por Maestro:</b>',readonly:'true',editable:false" labelWidth="130"></div>
          ...
          <div style="padding:10px 2px 10px 2px;"><a href="#pagar" class="easyui-linkbutton c6" iconCls="icon-ok" style="width:210px" onclick="abrirPago_128();">Pagar derecho de ceremonia</a>
          </div>
          <div style="padding:10px 2px 10px 2px;"><a href="#cancelar" class="easyui-linkbutton c6" iconCls="icon-cancel" style="width:210px" onclick="cancelPago_128();">Cerrar/Cancelar
              transaccion</a>
          </div>
        </div>
        <div data-options="region:'center'" style="width:730px;height:auto;">
          <iframe id="modalpg_128" class="embed-responsive-item" src="" width="98%" height="650" style="border:none;"></iframe>
        </div>
      </form>
    </div>
  </div>
