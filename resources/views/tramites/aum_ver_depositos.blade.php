...
<div class="easyui-layout" data-options="fit:true">
  ...
  <div class="datagrid-toolbar" id="toolbar{{ $_mid }}" style="display:inline-block">
    <div style="float:left;">
      @if (count($logias) > 1)
        <select id="fil_{{ $_mid }}" name="fil_{{ $_mid }}" class="easyui-combobox" data-options="panelHeight:600,valueField: 'id',textField: 'text',editable:false,onChange: function(rec){fDeposAum{{ $_mid }}(rec,'taller');}">
      ...
      @else
        <select id="fil_{{ $_mid }}" name="fil_{{ $_mid }}" class="easyui-combobox" data-options="panelHeight:'auto',valueField: 'id',textField: 'text',editable:false,onChange: function(rec){fDeposAum{{ $_mid }}(rec,'taller');}">
          @foreach ($logias as $key => $logg)
    ...
    <div style="float:left;"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="fa fa-check-square fa-lg correcto" onclick="revDatos_{{ $_mid }}();">Ver comprobantes registrados</a></div>
  </div>

</div>
<script type="text/javascript">
  function fDeposAum{{ $_mid }}(value, campo) {
  ...
  function revDatos_{{ $_mid }}() {
    ...
    if (row) {
      ...
      urldoca = '{{ $_folder }}/comprobantes/' + row.depositoGDR;
      urldocb = '{{ $_folder }}/comprobantes/' + row.depositoGLB;
      $("a#comprogdr").attr('href', urldoca);
      $("a#comproglb").attr('href', urldocb);
      $('#dlgauun{{ $_mid }}').dialog('open').dialog('setTitle', 'Datos de ceremonia para Aumento de salario');
...
<div id="dlgauun{{ $_mid }}" class="easyui-dialog" style="width:450px;height:auto;" closed="true" buttons="#dlgauun-buttons{{ $_mid }}" data-options="iconCls:'icon-save',modal:true">
  <form id="fmini0{{ $_mid }}" method="post" novalidate>
    ...
    <div class="easyui-panel" title="Revisar derechoas" style="width:100%;padding:5px;">
      <div style="margin-bottom:5px"><b>Deposito GLB(800) :</b><a href="" target="_blank" id="comproglb" class="enlace"> <br>Ver Imagen/PDF de Comprobante de deposito GLB</a></div>
      <div style="margin-bottom:5px"><b>Deposito GDR(875) :</b><a href="" target="_blank" id="comprogdr" class="enlace"> <br>Ver Imagen/PDF de Comprobante de deposito GDR</a></div>
