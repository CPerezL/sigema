...
<table id="dg{{ $_mid }}" class="easyui-datagrid" style="width:100%;height:100%;" data-options="url: '{{ $_controller }}/get_datos',
  ...
  <thead>
    <tr>
      ...
      <th field="sumaPagos" sortable="false">Suma Pagos</th>
    </tr>
  </thead>
</table>

<div class="datagrid-toolbar" id="toolbar{{ $_mid }}" style="display:inline-block">
  <div style="float:left;"><b>&nbsp;&nbsp;Gestion: </b><input id="gestion{{ $_mid }}" class="easyui-numberspinner" style="width:80px;" required="required" data-options="min:2010,max:{{ $year + 5 }},editable:false" value="{{ $year }}"></div>
  ...
  <div style="float:left;">
    <select id="filtro{{ $_mid }}" name="filtro{{ $_mid }}" class="easyui-combobox" data-options="panelHeight:'auto',valueField: 'id',textField: 'text',editable:false" style="width:220px;">
      <option value="1">Todos los aprobados por la GLB </option>
      <option value="2">Revisados por la GDR </option>
      <option value="3">Sin revisar por la GDR </option>
    </select>
  </div>
...
<script type="text/javascript">
  function r_habilitados{{ $_mid }}() {
    ...
    var gesreas = $('#gestion{{ $_mid }}').val();
    var tipofil = $('#filtro{{ $_mid }}').val();
    if (valle > 0) {
