...
<body>
    ...
    <table width="100%" cellspacing="0" cellpadding="2" border="1" align="center" style="font-size: 9px;">
      ...
      <tbody>
        ...
        foreach ($lista as $ver) {
            echo "<tr style=\"text-align:center;\"><td style=\"text-align:left;\" width=\"200\">$ver->NombreCompleto</td><td>$ver->grado</td><td>$ver->miembro</td><td>$ver->numeroCuotas</td><td width=\"80\">$ver->fechaUno/$ver->fechaDos</td><td></td><td>$ver->montoGDR</td><td>$ver->montoGLB</td><td>$ver->montoCOMAP</td><td>$ver->monto</td></tr>";
        }
        ?>
      </tbody>
      <tfoot>
      ...
      <table width="90%" cellspacing="0" cellpadding="3" border="1" align="center">
        <thead>
          <tr style="text-align: center;">
            ...
            <th>CANTIDAD</th>
            <th>IMPORTES GLB</th>
            <th>IMPORTES COMAP</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>Importe HH:. Regulares</td>
            <td style="text-align: center;">{{ (int) $mregu->canti }}</td>
            <td style="text-align: center;">{{ (int) $mregu->mglb }}</td>
            <td style="text-align: center;">{{ (int) $mregu->mcom }}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="text-align: center;">{{ (int) ($mregu->mglb + $mhono->mglb + $mause->mglb) }}</td>
            <td style="text-align: center;">{{ (int) ($mregu->mcom + $mhono->mcom + $mause->mcom) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <footer>
    <div style="text-align:right"><small>Emitido por el sistema de obolos SIGEMA/MECOM</small></div>
  </footer>
</body>
